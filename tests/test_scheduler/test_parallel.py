...
# 模拟采集器配置：name -> (collector_class, sleep_seconds)
MOCK_COLLECTORS = {
...
class TestParallelExecution:
    ...
    def test_parallel_faster_than_serial(self, mock_list, mock_get):
        """并行执行总耗时应远小于串行执行"""
    ...
    @patch("src.scheduler.tasks.get_collector", side_effect=_mock_get_collector)
    @patch("src.scheduler.tasks.list_collectors", side_effect=_mock_list_collectors)
    def test_parallel_subset_collectors(self, mock_list, mock_get):
        """并行执行指定采集器子集"""
        from src.scheduler.tasks import run_parallel_collectors
    ...
    @patch("src.scheduler.tasks.get_collector", side_effect=_mock_get_collector)
    @patch("src.scheduler.tasks.list_collectors", side_effect=_mock_list_collectors)
    def test_parallel_single_collector(self, mock_list, mock_get):
        """单个采集器并行执行（退化为串行）"""
        from src.scheduler.tasks import run_parallel_collectors
    ...
    @patch("src.scheduler.tasks.get_collector", side_effect=_mock_get_collector)
    @patch("src.scheduler.tasks.list_collectors", side_effect=_mock_list_collectors)
    def test_parallel_partial_failure(self, mock_list, mock_get):
        """并行执行中部分采集器失败不影响其他"""
        from src.scheduler.tasks import run_parallel_collectors
    ...
    @patch("src.scheduler.tasks.get_collector", side_effect=_mock_get_collector)
    @patch("src.scheduler.tasks.list_collectors", side_effect=_mock_list_collectors)
    def test_parallel_result_structure(self, mock_list, mock_get):
        """验证返回结果结构正确"""
        from src.scheduler.tasks import run_parallel_collectors
