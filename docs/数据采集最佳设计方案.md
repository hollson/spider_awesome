...
**代码即配置**：采集器的元数据（名称、启用状态、cron 调度、超时时间等）统一在 `configs/source_define.py` 中通过 `SourceMeta` 数据类注册。
...
SOURCES = [
    # ... 已有采集器
    SourceMeta(name="mymusic", cron="0 */6 * * *", timeout=600),
]
```
