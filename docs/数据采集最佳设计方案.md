...
```bash
spider_awesome/
├── configs/                   #  环境配置（仅 .env，无 Python 代码）
│   ├── .env                   #  全量脱敏基础配置
│   ├── .env.dev               #  开发环境专属
│   ├── .env.test              #  测试环境专属
...
2. **原始数据与处理数据物理隔离**：`var/database` 目录存放数据库，`var/raw` 存放原始缓存，`var/logs` 存放日志
...
| 方向 | 方案 |
|------|------|
| 新增采集器 | 继承 `BaseCollector`，实现 `fetch()` 方法，注册到 `registry.py` |
...
3. **配置分离**：`.env` 系列文件存配置，`settings.py` 统一读取
4. **调度兜底**：APScheduler 常驻 + 并行支持，适配各种场景
5. **单条容错**：异常捕获粒度到单条数据，失败不阻断整批任务
6. **环境隔离**：dev/test/prod 三套配置，零代码切换
