...
from app.collector import get_collector, list_collectors, list_enabled_collectors
...
def cmd_scheduler(args):
    """启动定时调度"""
    _print_banner()
    from app.collector import get_collector_schedule
    from app.scheduler.task_manager import task_manager
    from app.scheduler.tasks import parse_cron_to_hour_minute, run_all_collectors, run_parallel_collectors
...
def cmd_list(args):
    """列出所有采集器"""
    from app.collector.registry import get_collector_schedule
...
def cmd_logs(args):
    ...
    from app.storage.models import CollectLog
    from app.storage.session import get_db_instance
...
def _print_banner():
    """打印启动横幅"""
    from app.common.color import banner_label, banner_line, banner_title
